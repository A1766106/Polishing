...
{//intialise ID and Password
    ...
    string Temp_Type;
    string Line;
    while(getline(AccountsList, Line)) //while loop that keeps looping until ID and Password are given
    {
...
int Console::Create_Account()
{
    ...
    while (Account_Type != "1" && Account_Type != "2") //Input must be 1 or 2, or will be asked again
    {
    ...
    {
        Account_Number = Create_Broker(); //Account number equals the create Broker call.
        this->User_Type = 0; 
    }
    else
    {
        ...
        system("Clear"); //clear terminal
        cout << "Please select the number associated with the entity type you are: " << endl; //Output options
        cout << "1 - Individual" << endl;
        cout << "2 - Partnership" << endl;
        cout << "3 - Trust" << endl;
        cout << "4 - Company" << endl;
        cin >> Investor_Type; //Takes input from user
        while(Investor_Type != "1" && Investor_Type != "2" && Investor_Type != "3" && Investor_Type != "4") //If input is not 1,2,3,4 repeat Question
        {
            system("Clear"); //clear terminal
        ...
        {
            User_Type = 1; 
            Account_Number = Create_Individual(); //input 1 calls the create individual function
        }
        else if (Investor_Type == "2")
        {
            User_Type = 2;
            Account_Number = Create_Partnership(); //input 2 calls the create partnership function
        }
        else if (Investor_Type == "3") 
        {
            User_Type = 3;
            Account_Number = Create_Trust(); //input 3 calls the create trust function
        }
        else if (Investor_Type == "4") 
        {
            User_Type = 4;
            Account_Number = Create_Company(); //input 4 calls the create company function
        } 
...
int Console::Create_Broker()
{
    ...
    << "," << Recommendations [1] << "," << Recommendations [2] << "," << Client_Count; //display ID,Password,Type,Commission,Recommendations and client count.
...
int Console::Create_Partnership()
{
    ...
    string Partners_First_Names[Partner_Number]; //intialises partners first name with number attached
    string Partners_Last_Names[Partner_Number]; //intialises partners last name with number attached
    for(int i=0;i<Partner_Number;i++) //Loop through all the partners
    {
        ...
        cin >> Partners_First_Names[i]; //Take input of first name for current iteration of partner
        cout << "Please provide the last name for partner number " << i+1 << " as you would like it to appear on your portfolio statement: ";
        cin >> Partners_Last_Names[i]; //Take input of last name for current iteration of partner
    }
    fstream Accounts; //file object
    Accounts.open("Accounts.csv", fstream::app); //open accounts csv
    Accounts << Given_ID << "," << Given_Password << "," << "P" << "," << Partner_Number; //Display details for partners
    for(int i=0;i<Partner_Number;i++) //Loop through partners and display their names
    {
...
int Console::Create_Trust()
{
    ...
    string Beneficiary_First_Names[Beneficiary_Number]; //String for names with the size being the amount of beneficiaries there are
    string Beneficiary_Last_Names[Beneficiary_Number]; //String for names with the size being the amount of beneficiaries there are
    for(int i=0;i<Beneficiary_Number;i++) { //loop through from 0 to the amount of beneficiaries
    ...
    string Trustee_First_Names[Beneficiary_Number]; //String for names with the size being the amount of trustees there are
    string Trustee_Last_Names[Beneficiary_Number]; //String for names with the size being the amount of trustees there are
    for(int i=0;i<Trustee_Number;i++) //loop through from 0 to size of trustees number
    {
    ...
    fstream Accounts; //file object
    Accounts.open("Accounts.csv", fstream::app); //open accounts csv
    Accounts << Given_ID << "," << Given_Password << "," << "T" << "," << Beneficiary_Number; //display details
    for(int i=0;i<Beneficiary_Number;i++) //Loop through beneficiaries
    {
        Accounts << "," << Beneficiary_First_Names[i] << "," << Beneficiary_Last_Names[i]; //Display beneficiaries names
    }
    Accounts << ", " << Trustee_Number; //display details
    for(int i=0;i<Trustee_Number;i++)
    {
        Accounts << "," << Trustee_First_Names[i] << "," << Trustee_Last_Names[i]; //displays trustees names in accounts
    }
    Accounts << ",,,\n";
    Accounts.close(); //call close function
...
int Console::Create_Company()
{
    ...
    string Director_First_Names[Director_Number]; //string for directors names, Size = amount of directors
    string Director_Last_Names[Director_Number]; //string for directors names, Size = amount of directors
    for(int i=0;i<Director_Number;i++) //Loop Through directors
    {
        ...
        cin >> Director_First_Names[i]; //input director name for current iteration
        cout << "Please provide the last name for director number " << i+1 << " as you would like it to appear on your portfolio statement: ";
        cin >> Director_Last_Names[i]; //input director name for current iteration
    }
    fstream Accounts; //file object
    Accounts.open("Accounts.csv", fstream::app); //Open accounts csv
    Accounts << Given_ID << "," << Given_Password << "," << "C" << "," << Registered_Name << "," << Director_Number; //display details
    for(int i=0;i<Director_Number;i++) //Loop through directors
    {
        Accounts << "," << Director_First_Names[i] << "," << Director_Last_Names[i]; //Display names of directors
    }
    Accounts << ",,,\n";
    Accounts.close();
...
int Console::Get_ID()
{
    ...
    while(Duplicate_ID(ID)) //While the user inputs a duplicate ID, keep looping.
    {
