...
{
    cout << "Company name is: " << Registered_Name << endl; //Output Company name
    for(int i=0;i<Number_Of_Directors;i++) //Loop through directors
    {
        cout << "First name: " << Director_First_Name[i] << " Last name: " << Director_Last_Name[i] << endl; //Outputs directors names
    }
...
void Company::Set_Director_Count()
{
    ...
    while(getline(Information, line))
    {
            ...
            if(Int_Temp_ID == Account_Number) //If Temp Id is equal to account number then..
            {
                Number_Of_Directors = stoi(Temp_Director_Count); //...THe number of directors is equal to the temp director count. Stoi converts to int
                Information.close(); 
                break;
            }
    }
...
void Company::Set_Names()
{
    ...
    while(getline(Information, line)) //While loop until Temp ID is equal to account number
    {
            stringstream ss(line);
            getline(ss, Temp_ID, ',');
            Int_Temp_ID = stoi(Temp_ID);
            ...
            if(Int_Temp_ID == Account_Number)
            {
                ...
                for(int i=0;i<Number_Of_Directors;i++) //loop through directors
                {
                    ...
                    Director_First_Name[i] = Temp_First_Name; //Set first name to temp first name
                    Director_Last_Name[i] = Temp_Last_Name; //Set last name to temp last name
                }
                Information.close(); 
                break;
            }
...
void Company::Print()
{
    ...
    Printed_Portfolio.open("Company Portfolio Report.csv"); //open Company Portfolio csv
    int Share_Count = Investors_Portfolio->Get_Share_Count(); //get share count from portfolio
    Printed_Portfolio << "Report for Company Investors" << endl;
    Printed_Portfolio << "Company Name: " << Registered_Name << endl;
    for(int i=0;i<Number_Of_Directors;i++) //Loop through directors
    {
        Printed_Portfolio << "Director Number: " << i+1 << endl; //Display director number
        Printed_Portfolio << "\t First Name: " << Director_First_Name[i] << endl; //Display director name
        Printed_Portfolio << "\t Last Name: " << Director_Last_Name[i] << endl; //Display director name
    }
    for(int i=0;i<Share_Count;i++) //loop through shares
    {        
        int Temp_Units = Investors_Portfolio->Units(i); //Set temp units to portfolio units
        int Temp_Current_Price = Investors_Portfolio->Current_Price(i); //Set temp current price to portfolio current price
        int Temp_Previous_Price = Investors_Portfolio->Previous_Price(i); //Set temp previous price to portfolio previous price
        string Temp_Name = Investors_Portfolio->Name(i); //Set temp name to current name from portfolio
        Printed_Portfolio << "Share Number " << i+1 << " - " << Temp_Name << endl; //print share number
        Printed_Portfolio << "\t Units: " << Temp_Units << endl; //Print units
        Printed_Portfolio << "\t Current Price: $" << Temp_Current_Price << endl; //Prints current prices
        Printed_Portfolio << "\t Previous Price: $" << Temp_Previous_Price << endl; //Prints previous price
        Printed_Portfolio << "\t 12 Month Price Change: $" << (Temp_Current_Price-Temp_Previous_Price)*Temp_Units << endl; //prints 12 month change
    }
    Printed_Portfolio << "Your Bank Balance is furthermore: $" << Investors_Portfolio->Get_Cash() << endl; //print balance
    Printed_Portfolio.close();
    cout << "Please find your report called \"Company Portfolio Report\" " << endl; //Find it in company portfolio csv
