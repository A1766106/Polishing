...
Partnership::Partnership()
{
    ...
    Investors_Portfolio = new Portfolio; //Allocate memory for investors portfolio
}
...
{
    for(int i=0;i<Number_Of_Partners;i++) //For loop from 0  to the number of partners
    {
        cout << "First name: " << First_Name[i] << " Last name: " << Last_Name[i] << endl; //Output names of partners
    }
...
void Partnership::Set_Partners_Count()
{
    ...
    while(getline(Information, line)) //While loop to find ID
    {
            ...
            if(Int_Temp_ID == Account_Number) //if ID is equal to account number set number of partners
            {
                Number_Of_Partners = stoi(Temp_Partner_Count); //Convert number of partners to int
                Information.close(); 
                break;
            }
    }
...
void Partnership::Set_Partners_Names()
{
   ...
   First_Name = new string[Number_Of_Partners]; //Allocate memory for partners first names
   Last_Name = new string[Number_Of_Partners]; //Allocate memory for partners last names
    while(getline(Information, line)) //while loop to find Temporary ID
    {
            ...
            getline(ss, Temp_Nothing, ',');
            getline(ss, Temp_Nothing, ',');
            if(Int_Temp_ID == Account_Number) //If Temp ID equals the account number, set names.
            {
                for(int i=0;i<Number_Of_Partners;i++) //Loop through all the partners
                {
...
void Partnership::Print()
{
    ...
    Printed_Portfolio.open("Partnership Portfolio Report.csv"); //Open Partnership portfolio csv
    ...
    for(int i=0;i<Number_Of_Partners;i++) //For loop that will loop through the number of partners
    {
        Printed_Portfolio << "Partner Number: " << i+1 << endl; //Print partner number
        Printed_Portfolio << "\t First Name: " << First_Name[i] << endl; //Print first name
        Printed_Portfolio << "\t Last Name: " << Last_Name[i] << endl; //Print last name
    }
    for(int i=0;i<Share_Count;i++) //For loop will loop iterate all the shares
    {        
        int Temp_Units = Investors_Portfolio->Units(i); //Set temp units
        int Temp_Current_Price = Investors_Portfolio->Current_Price(i); //Set temp current price
        int Temp_Previous_Price = Investors_Portfolio->Previous_Price(i); //Set temp previous price
        ...
        Printed_Portfolio << "\t Current Price: $" << Temp_Current_Price << endl; //Print current price
        Printed_Portfolio << "\t Previous Price: $" << Temp_Previous_Price << endl; //Print previous price
        Printed_Portfolio << "\t 12 Month Price Change: $" << (Temp_Current_Price-Temp_Previous_Price)*Temp_Units << endl; //Print 12 month chanve
    }
    ...
    cout << "Please find your report called \"Partnership Portfolio Report\" " << endl; //Advises user where to find report
