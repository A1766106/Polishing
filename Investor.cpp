    ...
    { 
        {
            ...
            {
                system("Clear");
                cout << "You didnt use a valid number! Please try again! " << endl; //While the User does not input a valid number, show this message
            ...
            {
                if(Current_Year == Investors_Portfolio->Get_Year()) //if input is 2, print yearly statement
                {
                    system("Clear");
                    cout << "Sorry this option isn't avaliable when you are in the 2014 Financial Year! " << endl; //if year is 2014, this is unavaliable.
            ...
            else if(Option == "4") //If input is 4 then..
            {
                ...
                cout << "Please provide the Ticker of the stock you would like the current price of: "; //Output message
                cin >> Given_Ticker; //Take input from user
                Share Temp_Share(Given_Ticker,Current_Year); //Temp share object
                if(Temp_Share.Get_Name() != "N/A") //If ticker corresponds to actual ticker
                {
                    cout << "Stock Name: " << Temp_Share.Get_Name() << endl; //Show stock name
                    cout << "Current price: " << Temp_Share.Get_Price(Investors_Portfolio->Get_Year()) << endl; //Show current price
            ...
            else if(Option == "5") //Input 5 will clear terminal
            {
                int Temp_ID;
                system("Clear");
                cout << "What is the ID of the broker whos recommendations you would like to see? "; //It will then display this message
                cin >> Temp_ID; //take input of ID
                while(!Broker_ID_Confirmation(Temp_ID) && Temp_ID != -1) //Keep looping until a valid ID is entered
                {
                    system("Clear"); //clear terminal
                ...
                if(Broker_ID_Confirmation(Temp_ID)) //if ID correlated with a broker then display that the broker was set.
                {
                    system("Clear");
                    Set_Broker(Temp_ID);
            ...
            else if(Option == "6") //If input is 6, clear terminal and remove broker
            {
                system("Clear"); 
                Remove_Broker();
            }
            ...
            else if(Option == "8") //if input is 8, clear terminal and execute buy function
            {
                system("Clear");
                Buy();
            }
            else if(Option == "9") //if input is 9, clear terminal and execute sell function
            {
                system("Clear");
                Sell();
            }
...
void Investor::Get_Recommendations()
{
    ...
    string Temp_Type;
    while(getline(Information, line)) //While loop to match temp ID to broker ID
    {
    ...
    {
        cout << Temp_Recommendation[i] << endl; //Output recommendation in current iteration
    }
    cout << "Press enter to return to main page" << endl;
    cin.ignore();
...
{
    if(Transaction > Balance) //If transaction is greater than the balance return true, else return false
    {
        return true;
    }
    return false;
...
void Investor::Buy()
{
    ...
    if(Temp_Share.Check_Valid_Ticker(Temp_Ticker,Current_Year)) //Checking if ticker is valid 
    {
        Investors_Portfolio->Buy_Share(Temp_Ticker, Current_Year, Temp_Units); //add share to portfolio
    }
    else
    {
        cout << "That was an invalid ticker!" << endl; //else it was an invalid ticker
        this_thread::sleep_for(chrono::seconds(1)); //display for 1 second
    }
...
void Investor::Sell()
{
    ...
    Investors_Portfolio->Sell_Share(Temp_Ticker, Current_Year, Temp_Units); //Sell Share, remove from portfolio
...
bool Investor::Broker_ID_Confirmation(int ID)
{
    ...
    while(getline(Information, line))
    {
            ...
            getline(ss, Temp_Nothing, ',');
            getline(ss, Temp_Type, ',');
            if(Int_Temp_ID == ID && Temp_Type == "B") //If temp ID = ID and Type = B, then this confirms this is the broker
            {
                Information.close();
