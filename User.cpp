...
void User::Set_Account_Type()
{
    ...
    if(!Information.is_open()){
        cout << "File wasnt found and openned " << endl; //If statement determined file can not be opened
    }
    ...
    while(getline(Information, line))
    {
            ...
            Int_Temp_Account_Number = stoi(Temp_Account_Number); //set int temp account number to string temp account number and convert using stoi.
            if(Int_Temp_Account_Number == this->Account_Number) //if the temp number is equal to the actual account number...
            {
                Account_Type = Temp_Account_Type; //...Then the account type is equal to the temp type.
                break;
            }
    }
