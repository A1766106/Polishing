...
Portfolio::Portfolio()
{
    ...
    cout << "How much cash would you like to start with? "; //Output message to user
...
void Portfolio::Buy_Share(std::string Ticker, int Current_Year, int Units)
{
    ...
    if(!Reached_Share_Max())
    {
        ...
        if(Cash > (Temp_Share.Get_Price(Current_Year)*Units))
        {
            ...
            {
                User_Shares[Share_Count] = Share(Ticker, Current_Year, Units); //Add share to user shares.
                cout << "You have bought " << User_Shares[Share_Count].Get_Name() << endl; //Display the share the user bought
                this_thread::sleep_for(chrono::seconds(2)); //Display for 2 seconds
                Cash = Cash - (User_Shares[Share_Count].Get_Price(Current_Year)*Units); //User Balance = Balance minus (Current price multiplied by units).
                Share_Count++; //iterate share ocunt
            }
            else
            {
                int Temp_Units = User_Shares[Index_For_Stock].Get_Units(); //Set units for stock
                User_Shares[Index_For_Stock] = Share(Ticker, Current_Year, Units+Temp_Units); //Add share to user shares
                Cash = Cash - (User_Shares[Index_For_Stock].Get_Price(Current_Year)*Units); //User Balance = Balance minus (Current price multiplied by units).
                cout << "You have bought " << User_Shares[Index_For_Stock].Get_Name() << endl; //Display message of the bought share
                this_thread::sleep_for(chrono::seconds(2)); //display for 2 seconds
            }
        }
        else
        {
            //else statement that is executed when the user does not have enough money
    ...
    {
        //else statement that is executed when the user when maximum shares is reached
...
void Portfolio::Sell_Share(std::string Ticker, int Current_Year, int Units)
{
    ...
    for(int i=0;i<Share_Count;i++) //for loop that will loop from 0 to the share count
    {
        if(User_Shares[i].Get_Ticker() == Ticker) 
        {
            if(User_Shares[i].Get_Units() < Units) //if statement that will be executed when the user does not have enough units to sell
            {
            ...
            else
            {
                ...
                this_thread::sleep_for(chrono::seconds(2)); //Display for 2 seconds
                Cash = Cash + (User_Shares[i].Get_Price(Current_Year)*Units); //Balance = Balance + (Current price*Units)
...
{
    if(Share_Count > 20) //if share count is greater than 20, then the share limit has been reached
    {
        return true;
    }
    else
...
void Portfolio::Display_Portfolio()
{
    ...
    cout << "Your Portfolio is as Follows, Please Note Figures Have Been Rounded For Ease" << endl; //output message
    string temp;
    //This for loop will loop from 0 to the share count and output the share names and variables.
    for(int i=0;i<Share_Count;i++)
    {
        int Temp_Units = User_Shares[i].Get_Units(); //Get units
        int Temp_Current_Price = User_Shares[i].Get_Price(Current_Year); //Get current price
        int Temp_Previous_Price = User_Shares[i].Get_Price(Current_Year-1); //Get previous price
        cout << i+1 << " - Name: " << User_Shares[i].Get_Name() << " Current Stock Price: $" << User_Shares[i].Get_Price(Current_Year) << " Last Years Stock Price: $" << User_Shares[i].Get_Price(Current_Year-1) << " Units Held: " << User_Shares[i].Get_Units() << " Current Market Value: $"; //Output Share values
        cout << Temp_Units * Temp_Current_Price << " 12 Month Stock Change: $" << (Temp_Units*Temp_Current_Price) - (Temp_Units*Temp_Previous_Price) << endl; //Calculate and output the 12 month price change
    }
    cout << "Your Bank Balance is furthermore: $" << Cash << endl; //output Balance
...
void Portfolio::Increase_Current_Year()
{
    if(Reached_Year_Upper_Bound()) 
    {
        ...
        for(int i=0;i<Share_Count;i++) //for loop that will loop from 0 to the share count
        {
            User_Shares[i].Set_Current_Year(Current_Year); //Set user shares to new current year
        }
        cout << "You have advanced forward into year " << Current_Year << endl; //Output 
        this_thread::sleep_for(chrono::seconds(3)); //Display for 3 seconds
    }
    else
    {
        cout << "You cant advance any further you have reached the upper limit!" << endl; //If reached 2023, display this
        this_thread::sleep_for(chrono::seconds(1)); //display for 1 second
    }
}
...
void Portfolio::Decrease_Current_Year()
{
    if(Reached_Year_Lower_Bound())
    {
        ...
        cout << "You have advanced backward into year " << Current_Year << endl; //Output
        //for loop that will loop from 0 to the share count
        for(int i=0;i<Share_Count;i++)
        {
            User_Shares[i].Set_Current_Year(Current_Year); //Set user shares to the new current year
        }
    }
    else
    {
        cout << "You cant advance any further into the past you have reached the lower limit!" << endl; //Can not got further back than 2014
    }
}
...
bool Portfolio::Reached_Year_Upper_Bound()
{
    ...
    {
        return false; //if current year is greater than or equal to 2022, return false
    }
