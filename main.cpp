...
int main()
{
    ...
    if(UserConsole.Get_Existing_User() == 1) //Opens program to user
    {
        ...
        if(Account_ID == -1)
        {
            cout << "Login Failed! This Software Will Now Close!" << endl; //login fails as -1 is an invalid input
            UserConsole.Loading(); // display loading 
        }
        else
        {
            cout << "User ID Number: " << Account_ID << " Succesfully Logged In. " << endl; //Successful login as ID matched the csv file
            UserConsole.Loading(); //display loading
        }
    }
    else
    {
        ...
        cout << "User ID Number " << Account_ID << " Succesfully Created And Logged In! " << endl; //Output, registration was successful
        UserConsole.Loading(); // display loading
    }
    ...
    if(UserConsole.Logged_In())
    {
        ...
        else if (UserConsole.Get_User_Type() == 1) //if input is 1, then the investor is an individual
        {
            Individual U1(Account_ID);
        }
        else if (UserConsole.Get_User_Type() == 2) //if input is 2, then the investor is a partnership
        {
            Partnership U1(Account_ID);
        }
        else if (UserConsole.Get_User_Type() == 3) //if input is 3, then the investor is a trust
        {
            Trust U1(Account_ID);
        }
        else if (UserConsole.Get_User_Type() == 4) //if input is 4, then the investor is a company
        {
