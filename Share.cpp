...
Share::Share(string Ticker, int Current_Year)
{
    this->Ticker = Ticker;
    int Continue = 1;
    //While loop will keep looping until a valid ticker is entered eg. ANZ or user has elected to stop the function "Stop."
...
Share::Share(std::string Ticker, int Current_Year, int Units)
{
    ...
    //Set units
    this->Units = Units;
    Value = Get_Current_Price() * Units; //The value is the current price multiplied by the units
    Cost_Base = Get_Current_Price() * Units; //The cost base is the current price multiplied by the units
...
//function that will remove units once sold. Units will now equal units minus sold units.
...
std::string Share::Find_Name(int Year)
{
    ...
    else
    {
        ...
        if(!Information.is_open()){
            cout << "File wasnt found and openned " << endl; //Display if file not found
        }
        //Set variables
        string Temp_Ticker;
        string Temp_Price;
...
void Share::Increase_Units(int Units)
{
    ...
    Cost_Base += Units*Get_Current_Price(); //Cost base = Units multiplied by current price
}
//Free up memory
Share::~Share()
{
