...
{
    Set_Commission(); //Call the following functions, these are the functions the brokers use
    Set_Recommendations();
    Set_Clients_Count();
    Set_Clients();
}
...
void Broker::Set_Commission()
{
    ...
    if(!Information.is_open()){
        cout << "File wasnt found and openned " << endl; //If statement checking if file can be opened, display this if it cant be
    }
    ...
    while(getline(Information, line))
    {
            ...
            if(Int_Temp_ID == Account_Number) //if the temp Id is equal to the account number...
            {
                Commission = stoi(Temp_Commission); //then set the commission to the temp commission and convert to a int using stoi.
...
void Broker::Set_Recommendations()
{
    ...
    while(getline(Information, line))
    {
            ...
            if(Int_Temp_ID == Account_Number) //if the temp Id is equal to the account number...
            {
                for(int i=0;i<3;i++) //...Create a for loop, loop through all the recommendations. 
                {
                    getline(ss, Temp_Recommendation, ',');
                   Recommendations[i] = Temp_Recommendation; //set the recommendations to the temp recommendations.
                }
                Information.close(); 
                break;
            }
...
void Broker::Set_Clients_Count()
{
    ...
    while(getline(Information, line))
    {
            ...
            getline(ss, Temp_Nothing, ',');
            getline(ss, Temp_Nothing, ',');
            if(Int_Temp_ID == Account_Number) //if temp id is equal to the account number...
            {
                for(int i=0;i<3;i++) //...then create a for loop that loops through all the clients
                {
                    getline(ss, Temp_Nothing, ',');
                }
                getline(ss, Temp_Client_Count, ',');
                Client_Count = stoi(Temp_Client_Count); //Client count becomes the temp client count. Converted from string to int with the use of stoi
                Information.close(); 
                break;
            }
    }
...
void Broker::Set_Clients()
{
    ...
    while(getline(Information, line))
    {
            ...
            getline(ss, Temp_Nothing, ',');
            getline(ss, Temp_Nothing, ',');
            if(Int_Temp_ID == Account_Number) //If the Temp ID is equal to the account number then...
            {
                for(int i=0;i<3;i++) 
                {
                    getline(ss, Temp_Nothing, ',');
                ...
                getline(ss, Temp_Nothing, ',');
                for(int i=0;i<Client_Count;i++) //create a for loop that loops through the clients
                {
                    getline(ss, Temp_Client_ID, ',');
                    Clients_ID[i] = stoi(Temp_Client_ID); //The client ID now equals the temp client ID. COnverted from string to int via stoi
                }
                Information.close(); 
                break;
            }
...
{
    cout << "Commission: " << Commission << endl; //display the commission the broker will take
    for(int i=0;i<3;i++) //create a for loop that will loop through the recommendations 
    {
...
{   
    ...
    { 
        ...
        {
            system("Clear"); //while the valid is input, clear terminal and ask again
            cout << "You didnt use a valid number! Please try again! " << endl; //ask for valid output
...
void Broker::Print_Clients()
{
    ...
    {
        int Temp_ID_To_Search = Clients_ID[i]; //temp id is equal to clients ID at current iteration
        ifstream Information; //file object
        Information.open("Accounts.csv"); //open accounts csv
        ...
        string Temp_Recommendation[3] = {"N/A"}; //create string array of recommendations
        ...
        while(getline(Information, line)) //while loop, keeps looping until ID is mathched
        {
                stringstream ss(line);
                getline(ss, Temp_ID, ',');
                Int_Temp_ID = stoi(Temp_ID);
                ...
                if(Int_Temp_ID == Temp_ID_To_Search) //if temp ID is equal to Temp ID to search continute to type
                {
                    if(Temp_Type == "B") //if temp type is broker then output the following:
                    {

                        cout << "Client Number: " << i+1 << endl; //display client number
                        cout << "Client ID: " << Temp_ID_To_Search << endl; //display client ID
                        cout << "\t You have a broker saved in as a client and thus this information cant be displayed" << endl;
                        this_thread::sleep_for(chrono::seconds(1)); //disappear after 1 second
                    }
                    else if (Temp_Type == "I") //if an investor output the following
                    {
                        ...
                        getline(ss, Temp_Nothing, ',');
                        cout << "\t First Name: " << Temp_Nothing << endl; //display first name
                        getline(ss, Temp_Nothing, ',');
                        cout << "\t Last Name: " << Temp_Nothing << endl; //display last name
                    }
                    else if (Temp_Type == "P") //partnership
                    {
                        ...
                        for(int j=0;j<stoi(Temp_Partner_Count);j++) //loop through partners, convert string to int using stoi
                        {
                            cout << "\t Partner Number: " << j+1 << endl; //display partner number
                            getline(ss, Temp_Nothing, ',');
                            cout << "\t\t First Name: " << Temp_Nothing << endl; //display first name
                            getline(ss, Temp_Nothing, ',');
                            cout << "\t\t Last Name: " << Temp_Nothing << endl; //display last name
                    ...
                    else if (Temp_Type == "T") //trust
                    {
                        ...
                        for(int j=0;j<stoi(Temp_Bene_Count);j++) //loop through beneficiaries
                        {
                            cout << "\t Beneficiary Number: " << j+1 << endl; //display beneficiary number
                            getline(ss, Temp_Nothing, ',');
                            cout << "\t\t First Name: " << Temp_Nothing << endl; //display first name
                            getline(ss, Temp_Nothing, ',');
                            cout << "\t\t Last Name: " << Temp_Nothing << endl; //display last name
                        ...
                        for(int j=0;j<stoi(Temp_Trustee_Count);j++) //loop through trustees
                        {
                            cout << "Trustee Number: " << j+1 << endl; //display trustee number
                            getline(ss, Temp_Nothing, ',');
                            cout << "\t\t First Name: " << Temp_Nothing << endl; //display first name
                            getline(ss, Temp_Nothing, ',');
                            cout << "\t\t Last Name: " << Temp_Nothing << endl; //display last name
                    ...
                    else if (Temp_Type == "C") //company
                    {
                        ...
                        cout << "\t Client ID: " << Int_Temp_ID << endl; //display client id
                        getline(ss, Temp_Nothing, ',');
                        cout << "\t Company Name: " << Temp_Nothing << endl; //display company name
                        getline(ss, Temp_Dir_Count, ',');
                        for(int j=0;j<stoi(Temp_Dir_Count);j++) //loop through the directors
                        {
                            cout << "\t Director Number: " << j+1 << endl; //display director number
                            getline(ss, Temp_Nothing, ',');
                            cout << "\t\t First Name: " << Temp_Nothing << endl; //display first name
                            getline(ss, Temp_Nothing, ',');
                            cout << "\t\t Last Name: " << Temp_Nothing << endl; //display last name
...
bool Broker::Check_Add_Choice(int Choice)
{
    ...
    while(getline(Information, line))
    {
            stringstream ss(line);
            getline(ss, Temp_ID, ',');
            if(Choice == stoi(Temp_ID)) //if choice is equal to the temp ID return true.
            {
                Information.close();
...
void Broker::Swap_Clients()
{
    ...
    if(Check_Remove_Choice(Remove_Client)) //if statement to check if the removed client and choice are the same
    {
        ...
        if(Check_Add_Choice(Add_Client)) //add client to spot
        {
            ...
            {
                Replace_ID(Remove_Client, Add_Client); //swap the id of the removed client ID and added client
                cout << "These clients have been swapped! " << endl; //display for a second
                this_thread::sleep_for(chrono::seconds(1));
            }
            else
            {
                cout << "You already have this client! " << endl; //display for 1 second
                this_thread::sleep_for(chrono::seconds(1));
            }
        }
        else
        {
            cout << "This client doesn't exist! Returning to main menu! " << endl; //display for 1 second
            this_thread::sleep_for(chrono::seconds(1));
        }
    }
    else
    {
        cout << "You dont have this client! Returning to main menu! " << endl; //display for 1 second
        this_thread::sleep_for(chrono::seconds(1));
    }
}
...
void Broker::Print_To_CSV()
{
    ...
    cout << "Please find it under the name \"Broker Client Report\"" << endl; //displayed in csv
    ...
    for(int i=0;i<Client_Count;i++) //loop through clients
    {
        Printed_Client << "Client Number: " << i+1 << endl; //print client number
        ifstream Information; //file object
        Information.open("Accounts.csv"); //open accounts csv
        string Temp_ID; //create temp variable
        int Int_Temp_ID; //create temp variable
        ...
        while(getline(Information, line))
        {
                stringstream ss(line);
                getline(ss, Temp_ID, ',');
                Int_Temp_ID = stoi(Temp_ID); //int temp ID is equal to the string temp id. Convert using stoi.
                if(Int_Temp_ID == Clients_ID[i]) //if temp ID is equal to the clients ID and...
                {
                    getline(ss,Temp_Nothing, ',');
                    getline(ss,Account_Types, ',');
                    if(Account_Types == "B") //...the account is a broker then print the ID and following message
                    {
                    ...
                    {
                        Printed_Client << "\t Investor Type: Individual" << endl; //display type
                        Printed_Client << "\t Investor ID: " << Int_Temp_ID << endl; //display ID
                        getline(ss,Temp_Nothing,',');
                        Printed_Client << "\t\t First Name: " << Temp_Nothing << endl; //display first name
                        getline(ss,Temp_Nothing,',');
                        Printed_Client << "\t\t Last Name: " << Temp_Nothing << endl; //display last name
                    }
                    else if(Account_Types == "P") //Partnership
                    {
                        Printed_Client << "\t Investor Type: Partnership" << endl; //display type
                        Printed_Client << "\t Investor ID: " << Int_Temp_ID << endl; //display investor ID
                        getline(ss,Temp_Partner_Count,',');
                        for(int i=0;i<stoi(Temp_Partner_Count);i++) //loop through partners
                        {
                            Printed_Client << "\t\t Partner Number: " << i+1 << endl; //display partner number
                            getline(ss,Temp_Nothing,',');
                            Printed_Client << "\t\t\t First Name: " << Temp_Nothing << endl; //display first name
                            getline(ss,Temp_Nothing,',');
                            Printed_Client << "\t\t\t Last Name: " << Temp_Nothing << endl; //display last name
                    ...
                    else if(Account_Types == "T") //trust
                    {
                        ...
                        Printed_Client << "\t Investor ID: " << Int_Temp_ID << endl; //display ID
                        getline(ss,Temp_Trustee_Count,',');
                        for(int i=0;i<stoi(Temp_Trustee_Count);i++) //loop through trustees
                        {
                            Printed_Client << "\t\t Trustee Number: " << i+1 << endl; //display trustees number
                            getline(ss,Temp_Nothing,',');
                            Printed_Client << "\t\t\t First Name: " << Temp_Nothing << endl; //display first name
                            getline(ss,Temp_Nothing,',');
                            Printed_Client << "\t\t\t Last Name: " << Temp_Nothing << endl; //display last name
                        ...
                        getline(ss,Temp_Bene_Count,',');
                        for(int i=0;i<stoi(Temp_Bene_Count);i++) //loop through beneficiaries
                        {
                            Printed_Client << "\t\t Beneficiary Number: " << i+1 << endl; //display beneficiary number
                            getline(ss,Temp_Nothing,',');
                            Printed_Client << "\t\t\t First Name: " << Temp_Nothing << endl; //display first name
                            getline(ss,Temp_Nothing,',');
                            Printed_Client << "\t\t\t Last Name: " << Temp_Nothing << endl; //display last name
                    ...
                    {
                        Printed_Client << "\t Investor Type: Company" << endl; //display type
                        Printed_Client << "\t Investor ID: " << Int_Temp_ID << endl; //display ID
                        getline(ss,Temp_Nothing,',');
                        Printed_Client << "\t Company Name: " << Temp_Nothing << endl; //display company name
                        getline(ss,Temp_Dir_Count,',');
                        for(int i=0;i<stoi(Temp_Dir_Count);i++) //loop through directors
                        {
                            Printed_Client << "\t\t Director Number: " << i+1 << endl; //display director number
                            getline(ss,Temp_Nothing,',');
                            Printed_Client << "\t\t\t First Name: " << Temp_Nothing << endl; //display first name
                            getline(ss,Temp_Nothing,',');
                            Printed_Client << "\t\t\t Last Name: " << Temp_Nothing << endl; //display last name
