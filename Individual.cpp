...
void Individual::Get_Individual_Details()
{
    ...
    if(!Information.is_open()){
        cout << "File wasnt found and openned " << endl; //File unable to be opened
    }
    ...
    string line;
    while(getline(Information, line)) //while loop, loop until Temp ID is matched with account number
    {
...
void Individual::Print()
{
    ...
    Printed_Portfolio.open("Individual Portfolio Report.csv"); //open Individual portfolio csv
    int Share_Count = Investors_Portfolio->Get_Share_Count(); //Intialise share count
    Printed_Portfolio << "Report for Individual Investors" << endl; 
    Printed_Portfolio << "First Name: " << First_Name << endl; //display first name
    Printed_Portfolio << "Last Name: " << Last_Name << endl; //display last name
    for(int i=0;i<Share_Count;i++) //Loop from 0 to share count
    {        
        int Temp_Units = Investors_Portfolio->Units(i); //Set temp units to investors units
        int Temp_Current_Price = Investors_Portfolio->Current_Price(i); //Set temp current price to investors current price
        int Temp_Previous_Price = Investors_Portfolio->Previous_Price(i); //Set temp previous price to investors previous price
        string Temp_Name = Investors_Portfolio->Name(i); //Set temp name to investors name
        Printed_Portfolio << "Share Number " << i+1 << " - " << Temp_Name << endl; //Display share number
        Printed_Portfolio << "\t Units: " << Temp_Units << endl; //Display units
        Printed_Portfolio << "\t Current Price: $" << Temp_Current_Price << endl; //Display current price
        Printed_Portfolio << "\t Previous Price: $" << Temp_Previous_Price << endl; //Display Previous Price
        Printed_Portfolio << "\t 12 Month Price Change: $" << (Temp_Current_Price-Temp_Previous_Price)*Temp_Units << endl; //Display 12 month change
    }
    Printed_Portfolio << "Your Bank Balance is furthermore: $" << Investors_Portfolio->Get_Cash() << endl; //Display balance
    Printed_Portfolio.close(); //close portfolio
    cout << "Please find your report called \"Individual Portfolio Report\" " << endl; //Tells user to go to individual portfolio report csv
    this_thread::sleep_for(chrono::seconds(2)); //sleep for 2 seconds
}

void Individual::Individual_Display_Results_Test()
{
    cout << "Name: " << First_Name << " " << Last_Name << " balance: " << Balance << endl; //Display results
}
//Deconstructor
Individual::~Individual()
{
